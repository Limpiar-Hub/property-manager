    ...
    <div className="bg-white rounded-lg border">
      ...
      <div className="p-4 flex flex-col md:flex-row justify-between gap-4">
        <div className="relative">
          ...
          <input onChange={(e: any) => setSearchQuery(e.target.value)} value={searchQuery} type="text" placeholder="Search" className="pl-10 pr-4 py-2 border rounded-md w-full md:w-64" />
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {currentRows.map((transaction: any) => (
              <tr key={transaction.id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-4 text-sm">{new Date(transaction.createdAt).toDateString()}</td>
      ...
      <div className="p-4 flex flex-col md:flex-row justify-between items-center text-sm">
        <div className="flex items-center gap-2 mb-4 md:mb-0">
          ...
          <div className="relative w-16">
            <select
              ...
              value={rowsPerPage} className="w-full appearance-none border rounded-md px-2 py-1 pr-6 bg-white">
        ...
        <div className="flex gap-2">
          ...
          disabled={currentPage === 1}
           className="px-4 py-2 border rounded-md text-gray-500 disabled:text-gray-300 hover:bg-gray-50">Previous</button>
          <button
          onClick={() =>
            setCurrentPage((prev) =>
              Math.min(prev + 1, totalPages)
          ...
          disabled={currentPage === totalPages}
              className="px-4 py-2 border rounded-md bg-gray-900 disabled:bg-gray-500 text-white hover:bg-gray-800">Next</button>
        </div>
      </div>
    </div>
  )
