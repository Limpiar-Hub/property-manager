...
export default function PropertyBookings({ bookings }: PropertyBookingsProps) {
  ...
  const countActive = bookings.filter((book) =>
    book.status.toLowerCase().includes("confirm")
  );
  const countPending = bookings.filter((book) =>
    book.status.toLowerCase().includes("pend")
  );
  const countCancelled = bookings.filter((book) =>
    book.status.toLowerCase().includes("cancel")
  );
  const countCompleted = bookings.filter((book) =>
    book.status.toLowerCase().includes("complete")
  );
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="p-4 border-b flex items-center justify-between">
          <div className="text-sm text-gray-500">
            {selectedRows.length} column selected
          </div>
          <Button
            variant="outline"
            size="sm"
            ...
            <tbody>
              {bookings.map((booking) => (
                <tr key={booking._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    {booking.serviceType.split(" ").slice(0, 3).join(" ")}
                  </td>
                  <td className="p-4">{booking.propertyId?.name}</td>
                  <td className="p-4">{booking.date.slice(0, 10)}</td>
                  <td className="p-4">{booking.startTime}</td>
                  <td className="p-4">{booking.status}</td>
                </tr>
              ))}
            </tbody>
