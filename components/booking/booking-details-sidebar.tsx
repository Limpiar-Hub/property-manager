...
export default function BookingDetailsSidebar({
  ...
  const handleSendMessage = async () => {
    try {
      ...
      // Type-safe createChatThread dispatch
      const response = await dispatch(
        createChatThread({
          participantIds: [propertyManagerId!, cleanerId],
          taskId: booking._id,
      ...
      // Check the response using proper type checking
      // Access payload through the standard Redux Toolkit action structure
      const newChat = 'payload' in response ? response.payload : null;
